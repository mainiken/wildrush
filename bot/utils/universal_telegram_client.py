        ...
        if not self._webview_data:
            while True:
                try:
                    ...
                    # Параметр app обязательный для RequestAppWebView
                    if not bot_shortname:
                        # Если bot_shortname не указан, используем пустую строку как fallback
                        bot_shortname = ""
    ...
    async def _pyrogram_get_webview_url(self, bot_username: str, bot_url: str, default_val: str) -> str:
        ...
        async with self.lock:
            try:
                ...
                if not self.client.is_connected:
                    await self.client.connect()
                # Для обычного WebView инициализируем данные без app параметра
                peer = await self.client.resolve_peer(bot_username)
                self._webview_data = {'peer': peer, 'bot': peer}
                await asyncio.sleep(uniform(1, 2))
                ...
                if not start_state:
                    ...
                    await self.client.invoke(pmessages.StartBot(peer=peer,
                                                                bot=peer,
                                                                random_id=randint(1, 2**63),
                                                                **start))
                await asyncio.sleep(uniform(1, 2))
                if settings.DEBUG_LOGGING:
                    logger.debug(f"[{self.session_name}] RequestWebView params: peer={peer}, bot={peer}, start={start}, bot_url={bot_url}")
